...
При выполнении, следуй пожалуйста информации и просьбам:
...
- Server assemblies live in `apps/server/src/modules/*`, configs in `apps/server/src/config/*`, shared utilities under `apps/server/src/utils/*` (e.g., `crypto.ts`, `ip.ts`, `jwt.ts`). It exposes public REST APIs at `/api/v1/...`, internal endpoints under `/internal/...`, implements market & personal analytics, credit wallets, archiving, and background jobs.
- Client widgets and pages are under `apps/client/src/components/*`, `contexts/*`, `lib/*`. UI is built with Radix/shadcn components and communicates with the server via REST and tokens.
- All prompts are stored inside `/prompts` (one prompt per file). Documentation (README, CHANGELOG, MIGRATION NOTES) lives in `/docs`. Every level-1 and level-2 folder must contain its own `README.md` describing structure, purpose, integrations (frontend/backend/DB/API), scalability paths, optimizations, and dependencies.
- Ambiguity, duplicated logic/text, hacks, security gaps, or incomplete features are **strictly forbidden**. If something blocks completion, describe the blocker explicitly and stop instead of shipping placeholders.
...
- Design modularly with SRP & SOLID; prefer composition over inheritance and rely on DI rather than global singletons.
- Split layers into `domain`, `application`, `adapters/ports`, `infrastructure`, and keep their boundaries explicit.
...
- Each level-1/2 folder must own a `README.md` that concisely describes purpose, structure, interactions, scaling paths, optimizations, and collaboration with other modules or services.
...
- Place prompts strictly in `/prompts`, one per file.
- Documentation resides in `/docs`: README, CHANGELOG, MIGRATION NOTES.
...
- Whenever DB/ENV/docs change, add a MIGRATION NOTE block (area, rationale, steps: backup → migrate → seed → rollback, and impacts such as downtime/compatibility).
